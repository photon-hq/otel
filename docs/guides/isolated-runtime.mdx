...
marks the Span `ERROR`, ends it, and rethrows the same value without exporting
exception details. Record caller-curated public details explicitly while the
Span is active:
...
The runtime exports `type` as `error.type` and `exception.type`, plus the
optional `message` as `exception.message`. It never accepts or exports a raw
`Error` or stack trace. Export and processor failures do not replace the
business result.
