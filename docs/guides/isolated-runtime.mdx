...
An optional `idGenerator` replaces that provider's random trace and span ID
generator with any OpenTelemetry `IdGenerator`; it is forwarded unchanged, so
the SDK's own rules apply: every Span asks it for a span ID, and only a Span
without a parent asks it for a trace ID.
