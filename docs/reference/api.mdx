...
endpoint. The runtime creates its own Resource from `resourceAttributes`; an
omitted value produces an empty Resource. `traceparentHeader` must be a private
carrier name and cannot be the standard `traceparent` header.
