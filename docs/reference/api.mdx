...
interface IsolatedOtelErrorDetails {
  type: string;
  message?: string;
}
```
...
`recordError()` marks the active local Span `ERROR`, exports `type` as both
`error.type` and `exception.type`, and exports an optional caller-curated
`message` as `exception.message`. It does not accept or export raw `Error`
objects or stack traces. Invalid JavaScript input is a diagnostic no-op for
details and never changes business execution.
