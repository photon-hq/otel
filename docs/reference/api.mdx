...
`idGenerator` is the standard OpenTelemetry `IdGenerator` from
`@opentelemetry/sdk-trace-base`, forwarded unchanged to the isolated
`BasicTracerProvider`. The SDK calls `generateSpanId()` for every Span and
`generateTraceId()` only for Spans without a parent. Omit it to keep the SDK's
random generator.
