...
The low-level `recordError()` boundary accepts `unknown` and validates it at
runtime. A structured `{ type: string, message?: string }` value marks the
active local Span `ERROR`, exports `type` as both `error.type` and
`exception.type`, and exports an optional caller-curated `message` as
`exception.message`. It does not accept or export raw `Error` objects or stack
traces. Invalid input is a diagnostic no-op for details and never changes
business execution. Application-facing wrappers can expose a narrower input
type.
